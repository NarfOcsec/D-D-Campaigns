# Lista "scuola" — Tallyband demoni Rotigus + GUO (consigliata da top player)

> Consigliata all'utente da un top player per imparare la fazione. GHB 2026-27,
> Tallyband of Nurgle, 4 drops, 1990/2000 pts, 172 wounds.
> Battle Tactic Cards: Siege of Ashes, Flanking Firestorm. Spell: Malignance.
> Prayer: Benedictions of Sickness. Manifestation: Primal Energy (10 pt). Terreno: Gnarlmaw.
> ⚠️ Regole verificate su fonti web (Wahapedia, recensioni), non su materiale ufficiale.

## Lista

- **Reg 1 (General's)**: Rotigus (420, General) · 20 Plaguebearers (260) + The Weeping Flux ·
  20 Plaguebearers (260) · Sloppity Bilepiper (90)
- **Reg 2**: Great Unclean One (380) + The Witherstave + Gift of Febrile Frenzy ·
  3 Plague Drones (150) · 3 Plague Drones (150)
- **Reg 3**: Belga the Cystwitch (130)
- **Reg 4**: Horticulous Slimux (140)

Nota punti: totale dichiarato 1990 → o The Weeping Flux non costa più 10 pt, o avanzano
10 pt reali. Da ricontrollare sull'app prima di presentarla a un torneo.

## Perché è un'ottima lista per imparare la fazione

Contiene **tutti i sottosistemi Maggotkin in un'unica lista**, ognuno in versione semplice:

1. **Magia** — Rotigus + GUO, con Belga che dà +1 al cast in aura.
2. **Preghiere** — Belga + Benedictions of Sickness: si impara la fase chant senza la
   complessità dei Blightkings-preti.
3. **Malattia** — Tallyband (Plague Progenitor), Weeping Flux, Ruptured Cysts di Belga.
4. **Gioco del terreno** — Horticulous pianta un secondo Gnarlmaw (se ce n'è al massimo 1)
   e può teletrasportarli: insegna il map control alla Nurgle.
5. **Mobilità e tattiche** — 6 Plague Drones in due unità: servono Flanking Firestorm,
   obiettivi lontani, screening veloce. La cosa che manca a quasi tutte le altre build.
6. **Gestione dei blob** — 2×20 Plaguebearers: screening, tappi, obiettivi.
7. **Buff/debuff da posizionare** — Bilepiper (+2 run/charge a un demone, o D3 mortali a
   chi si allontana), Witherstave (nemici entro 12" ritirano i 6 per colpire = anti-crit),
   Febrile Frenzy (+1 Attacks su 3+).

È anche **perdonante**: 172 wounds, ward diffusi, Emerald Lifeswarm (Primal Energy) per
curare, ridondanza ovunque (2 caster, 2 blob, 2 unità di drones). Gli errori costano poco.

## Confronto con le altre due liste discusse

| | Lista utente (demoni) | Questa ("scuola") | Mestyanek 5-0 |
|---|---|---|---|
| Drops | 2 | 4 | 5 |
| Preghiere | no | sì (Belga) | sì (Belga + Blightkings SoA) |
| Mobilità | 3 Drones | 6 Drones + tricks Gnarlmaw | nessuna |
| Punch dei blob | alto (2× Poxbringer combo) | medio (Febrile Frenzy) | basso (blob solo tappo) |
| Difesa castello | ward + corpi | + Witherstave anti-crit | + screens sacrificabili |
| Complessità di pilotaggio | bassa | media | alta |

## Acquisti per l'utente (partendo dalla sua lista demoni)

Servono solo: **Sloppity Bilepiper, Belga, Horticulous Slimux, +3 Plague Drones**.
Riusa: Rotigus, GUO, 40 dei suoi 50 Plaguebearers, 3 Drones, Gnarlmaw.
Restano a casa: 2 Poxbringer, Rotswords, 10 Plaguebearers.

## Verdetto

Consiglio condiviso: è la palestra giusta. Stesso archetipo che l'utente già gioca, tutti i
sottosistemi della fazione presenti, pochissimi acquisti, e una volta imparata si può
evolvere verso la build alla Mestyanek aggiungendo il pacchetto mortal (Blightkings SoA,
Pox-Wretches, Harbinger) se serve più tech.

## Roadmap di apprendimento (l'utente ha già tutti i modelli)

Stessa lista, senza modifiche, per almeno 10 partite: il valore sta nella ripetizione.

- **Partite 1-2 — deployment e movimento.** Solo fondamentali: blob davanti, mostri
  dietro, mai farsi caricare per primi con Rotigus/GUO. Ignorare le finezze.
- **Partite 3-4 — economia della malattia.** Scelta del Plague Progenitor al deployment
  (Health bassa, o l'unità che l'avversario vorrà curare), sequenza chant di Belga,
  quando conviene il Weeping Flux.
- **Partite 5-6 — Gnarlmaw e Horticulous.** Dove piantare il secondo albero, usare
  run+charge dall'aura, teletrasporto per seguire il fronte.
- **Partite 7-8 — Drones e battle tactics.** Giocare Flanking Firestorm di proposito,
  rubare obiettivi laterali, screening veloce.
- **Partite 9-10 — rigiocare i matchup persi** e decidere se restare demoni o innestare
  il pacchetto mortal.

Cosa resta difficile a prescindere dalla lista (ed è il vero programma di studio):
screening, gestione del doppio turno, scoring delle tactics, timing del commit dei mostri.
